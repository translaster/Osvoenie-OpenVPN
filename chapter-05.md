...
Одним из недостатков использования _redirect-gateway_ является то, что адаптер VPN теперь является единственным адаптером со шлюзом по умолчанию и, следовательно, является доверенным. Адаптер беспроводной сети, подключенный к сети Wi-Fi `eduroam`, теперь не имеет маршрута по умолчанию и внезапно становится частью _неопознанной сети_ и больше не является доверенным. Это можно увидеть на следующем скриншоте **Центра управления сетями и общим доступом** Windows 7:
